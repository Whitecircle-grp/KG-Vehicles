...
async function sendWhatsapp(to, vehicleNumber, upcomingExpiries) {
  ...
  //  const expiryList = upcomingExpiries.join('%0A');
    const url = `https://bhashsms.com/api/sendmsgutil.php?user=white_circle&pass=123456&sender=BUZWAP&phone=${ph}&text=vehicle_expiry_alert&priority=wa&stype=normal&params=${vehicleNumber},${expiryList}`;
